  ...
  if (!response.ok) {
    ...
    try {
      if (response.headers.get('content-type')?.includes('application/json')) {
        ...
        if (/scope/.test(message)) reason = 'The selected API-key connection is missing the MCP scope.';
        else if (/not enabled|disabled/.test(message)) reason = 'MCP access is not enabled for this connection or account.';
        else if (/license|permission|access denied/.test(message)) reason = 'Seamless denied this connection access. Check its assigned group and account permissions.';
      }
    } catch {}
...
export class SeamlessMcpClient {
  get credentialId(): string { return crypto.createHash('sha256').update(this.apiKey).digest('hex'); }
  constructor(
