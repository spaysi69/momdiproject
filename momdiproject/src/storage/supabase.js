...
class SupabaseStore{
  ...
  async ensurePerson(url){const payload={mode:'direct-linkedin-rest',person:{linkedinUrl:url},updatedAt:new Date().toISOString()};await this.request('person_searches',{on_conflict:'normalized_url'},'POST',{normalized_url:url,payload,updated_at:new Date().toISOString()},'resolution=merge-duplicates,return=minimal')}
