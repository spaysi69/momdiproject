...
function parseMarkdownTable(text: string): Record<string, any>[] {
  ...
  for (let i = 0; i < lines.length - 2; i++) {
    ...
    const rows: string[][] = [];
    for (let j = i + 2; j < lines.length && lines[j].includes('|'); j++) rows.push(lines[j].replace(/^\|/, '').replace(/\|$/, '').split('|').map(x => x.trim()));
    for (const row of rows) {
      const obj: Record<string, any> = {};
      const aliases: Record<string,string> = {searchresultid:'searchResultId',fullname:'fullName',name:'fullName',jobtitle:'title',companyname:'company',company:'company',title:'title',email:'email',phone:'phone',linkedin:'linkedinUrl',linkedinurl:'linkedinUrl',companydomain:'companyDomain'};
...
export async function searchPersonByLinkedIn(mcp: SeamlessMcpClient, linkedinUrl: string): Promise<PersonSearchCandidate[]> {
  ...
  const field = ['liProfileUrl', 'linkedinUrl', 'liProfileUrls', 'linkedinUrls'].find(x => props[x]);
  if (!field) throw new Error('This MCP connection does not advertise LinkedIn URL filtering. LinkedIn lookup cannot run until Seamless enables that filter on your MCP connection. No research credits were used.');
  const raw = await mcp.searchContacts({[field]: props[field]?.type === 'array' ? [linkedinUrl] : linkedinUrl, limit: 100});
