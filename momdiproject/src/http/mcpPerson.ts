...
export async function searchPersonByLinkedIn(mcp: SeamlessMcpClient, linkedinUrl: string): Promise<PersonSearchCandidate[]> {
  ...
  const args = linkedInSearchArguments(tool?.inputSchema, linkedinUrl);
  if (!args) throw new Error('The app could not identify a supported LinkedIn URL filter in search_contacts. This is not an authorization error. Use Copy search diagnostic so the actual tool schema can be checked. No paid lookup was attempted.');
