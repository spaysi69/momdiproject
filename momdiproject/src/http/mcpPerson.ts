...
export async function searchPersonByLinkedIn(mcp: SeamlessMcpClient, linkedinUrl: string): Promise<PersonSearchCandidate[]> {
  ...
  if (!field) throw new Error('This MCP connection does not advertise LinkedIn URL filtering. Search by name and company instead.');
  const raw = await mcp.searchContacts({[field]: props[field]?.type === 'array' ? [linkedinUrl] : linkedinUrl, limit: 20});
  return parseMcpContacts(mcp.normalizeToolResult(raw), '');
}
