...
const PollEnvelopeSchema = z.object({
  ...
  data: z.array(z.object({
    ...
    // Seamless can return different shapes for these fields between records.
    // Keep the transport parser permissive and validate only what the app needs.
...
function readPollResult(payload: unknown): { status: string; message?: string; contact?: Record<string, any>; additionalData?: unknown } {
  const parsed = PollEnvelopeSchema.parse(payload);
  const result = parsed.data[0];
  const status = String(result.status ?? '').toLowerCase();
  const message = typeof result.message === 'string' ? result.message : undefined;
  const contact = result.contact && typeof result.contact === 'object' && !Array.isArray(result.contact)
    ? result.contact as Record<string, any>
    : undefined;
  return { status, message, contact, additionalData: result.additionalData };
}
