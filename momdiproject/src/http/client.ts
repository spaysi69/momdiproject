...
export class SeamlessClient {
  ...
  async searchPersonByLinkedInUrl(linkedinUrl: string, limit = 50): Promise<{ contact: ContactSearchResult }> {
    ...
    const results = await Promise.allSettled(queries.map(async body => {
      const response = await this.client.post('/search/contacts', body, { timeout: Math.min(this.options.timeoutMs, 10000) });
      this.throwIfBad(response, 200, 'contact search');
      return SearchEnvelopeSchema.parse(response.data).data.map((item: Record<string, unknown>) => item as ContactSearchResult);
    }));
    ...
    throw new Error('No exact LinkedIn-profile match was returned by Seamless Contact Search. No research credit was consumed.');
  }
