  ...
  async searchPersonByLinkedInUrl(linkedinUrl: string, limit = 50): Promise<{ contact: ContactSearchResult }> {
    ...
    for (const body of queries) {
      try {
        ...
        const parsed = SearchEnvelopeSchema.parse(response.data);
        const matches = parsed.data.map((item: Record<string, any>) => item as ContactSearchResult).filter((item: ContactSearchResult) => {
          const candidate = firstString(item.lIProfileUrl, (item as any).liProfileUrl, (item as any).linkedinUrl);
          return candidate ? candidate.trim().toLowerCase().replace(/\/$/, '') === normalized : false;
        });
        if (matches[0]) return { contact: matches[0] };
      } catch (error) {
        lastError = error;
      }
    }
    if (lastError && (lastError as any)?.response) {
      throw lastError;
    }
