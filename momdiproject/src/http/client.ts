...
function mapContact(contact: Record<string, any>, fallbackLinkedInUrl?: string): EnrichmentResponse {
  ...
  return {
    ...
    jobHistory: Array.isArray(contact.jobHistory) ? contact.jobHistory.filter((item: unknown) => item && typeof item === 'object') as Array<Record<string, unknown>> : undefined,
    raw: contact,
  };
}
...
export class SeamlessClient {
  ...
  async researchContactByLinkedInUrl(linkedinUrl: string, skipDeduplicationCheck = false, onAccepted?: () => Promise<void>): Promise<EnrichmentResponse> {
    const accepted = await this.client.post('/contacts/research', {
