...
export function classifyProviderError(error: any): ClassifiedError {
  ...
  if (status === 422 && /insufficient.?credits|credit|license/i.test(bodyText)) return { kind: 'CREDITS', retryable: false, status, message: 'Provider credits or license are unavailable' };
