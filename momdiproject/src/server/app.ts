...
export async function createApp() {
  ...
  app.post('/v1/person/companies', authenticate, async (req, res, next) => {
    try {
      ...
      if (error?.code === 'SEAMLESS_RESEARCH_PROCESSING') return res.status(202).json({ status: 'processing', jobId: error.jobId });
  ...
  app.post('/v1/person/companies/status', authenticate, async (req, res, next) => {
    try {
      const linkedinUrl = typeof req.body?.linkedinUrl === 'string' ? req.body.linkedinUrl.trim() : '';
      if (!linkedinUrl) return res.status(400).json({ error: 'Enter a LinkedIn person URL.' });
      const result = await enrichment.pollPersonLookup(linkedinUrl);
      if (result.status === 'not_found') return res.status(404).json({ error: 'No active person research job was found. Start the person lookup again.' });
      if (result.status === 'processing') return res.status(202).json(result);
      return res.json(result);
    } catch (error: any) {
      const classified = classifyProviderError(error);
      logger.warn('http.person_lookup.poll_failure', { kind: classified.kind, status: classified.status, message: classified.message, stack: error?.stack });
      if (classified.kind === 'AUTH') return res.status(502).json({ error: `Provider authorization failed: ${classified.message}` });
      if (classified.kind === 'CREDITS') return res.status(402).json({ error: `Provider credits unavailable: ${classified.message}` });
      if (classified.kind === 'RATE_LIMIT') return res.status(503).json({ error: `Provider rate limit reached: ${classified.message}` });
      if (classified.kind === 'TRANSIENT') return res.status(503).json({ error: `Seamless temporary failure while polling: ${classified.message}` });
      return res.status(502).json({ error: error?.message ? `Seamless research polling failed: ${error.message}` : 'Seamless research polling failed.' });
    }
  });
