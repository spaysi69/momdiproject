...
export async function createApp() {
  ...
  app.post('/status/refresh-credits', authenticate, async (_req: express.Request, res: express.Response, _next: express.NextFunction) => {
    try {
      await enrichment.refreshProviderCredits();
      res.json(await enrichment.status());
    } catch (error: any) {
      logger.error('http.status.credit_refresh_failed', { message: error?.message || String(error), stack: error?.stack });
      res.status(502).json({ error: error?.message ? `Provider credit refresh failed: ${error.message}` : 'Provider credit refresh failed.' });
    }
  });
