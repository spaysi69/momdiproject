...
import { requireRedisUrl } from '../storage/redis';
...
export async function createApp() {
  ...
  redis.on('error', (err) => logger.error('redis.connection_error', { message: err.message }));
  try {
    await redis.connect();
    await redis.ping();
  } catch (error: any) {
    const safeHost = (() => { try { return new URL(redisUrl).host; } catch { return 'invalid-redis-url'; } })();
    logger.error('redis.startup_failed', { host: safeHost, message: error?.message ?? 'unknown error' });
    redis.disconnect();
    throw new Error(`Redis connection failed for ${safeHost}. Verify REDIS_URL uses the Render Key Value INTERNAL connection string and that both services are in the same region.`);
  }
  ...
  app.get('/ready', async (_req, res) => {
    try { await redis.ping(); res.status(200).json({ status: 'ready' }); }
