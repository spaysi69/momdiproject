...
export async function createApp() {
  ...
  app.post('/v1/resolve-company', authenticate, async (req, res, next) => {
    try {
      const companyLinkedInUrl = typeof req.body?.companyLinkedInUrl === 'string' ? req.body.companyLinkedInUrl.trim() : '';
      if (!companyLinkedInUrl) return res.status(400).json({ error: 'Enter a LinkedIn company URL.' });
      res.json(await enrichment.resolveCompanyByLinkedInUrl(companyLinkedInUrl));
    } catch (error: any) {
      if (error?.message?.startsWith('Enter a valid LinkedIn company URL') || error?.message?.includes('could not be matched')) return res.status(422).json({ error: error.message });
      next(error);
    }
  });
