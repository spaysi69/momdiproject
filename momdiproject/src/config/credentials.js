...
function seamlessCredentials(env=process.env){
  ...
  for(const name of Object.keys(env).filter(k=>/^SEAMLESS_API_KEY_[A-Z0-9_]+$/.test(k)&&clean(env[k])).sort()){
  ...
  if(selected){
    ...
    else candidates.splice(candidates.indexOf(selected),1),candidates.unshift(selected);
  }
  if(!candidates.length)throw new Error('Configure at least one Seamless Public API v1 key in Render.');
  const seenKeys=new Set();
  const routes=[];
  for(const source of candidates){
    const key=clean(env[source]);
    if(!key||seenKeys.has(key))continue;
    seenKeys.add(key);
    const proxySource=proxyVarForSource(source);
    const proxyUrl=proxySource?clean(env[proxySource]):'';
    routes.push({id:routeIdForSource(source),source,key,proxySource,proxyUrl:proxyUrl||null});
  }
  ...
  if(requireUniqueEgress&&routes.length>1){
    ...
    const proxies=routes.map(r=>r.proxyUrl);
    if(new Set(proxies).size!==proxies.length)throw new Error('Unique egress is required, but two Seamless keys are configured with the same proxy URL.');
  }
  return {routes,requireUniqueEgress};
}
function seamlessCredential(env=process.env){
  const {routes}=seamlessCredentials(env);const r=routes[0];return{source:r.source,key:r.key,proxyUrl:r.proxyUrl};
}
module.exports={seamlessCredential,seamlessCredentials,proxyVarForSource,routeIdForSource};
