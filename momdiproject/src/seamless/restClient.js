...
function diagnosticRecord({routeId,keySource,path,method,requestBody,status,responseHeaders,responseBody,startedAt,observedAt,error=null}){
  return safeJson({
    step:'seamless_http',routeId,keySource,startedAt,observedAt,
...
class SeamlessRestClient{
  constructor(apiKey,baseUrl='https://api.seamless.ai/api/client/v1',timeoutMs=30000,fetchImpl=null,options={}){if(!apiKey)throw new Error('Missing Seamless API key');this.apiKey=apiKey;this.baseUrl=baseUrl.replace(/\/$/,'');this.timeoutMs=timeoutMs;this.fetch=fetchImpl||fetch;this.routeId=options.routeId||'default';this.keySource=options.keySource||'SEAMLESS_API_KEY'}
  async request(path,init={}){
    const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),this.timeoutMs);const startedAt=new Date().toISOString();const method=String(init.method||'GET').toUpperCase();const requestBody=parseRequestBody(init.body);
    try{
      ...
      const observedAt=new Date().toISOString();const diagnostic=diagnosticRecord({routeId:this.routeId,keySource:this.keySource,path,method,requestBody,status:response.status,responseHeaders:sanitizeHeaders(response.headers),responseBody:safeJson(body),startedAt,observedAt});
      ...
      if(error?.name==='AbortError'){
        const observedAt=new Date().toISOString();const diagnostic=diagnosticRecord({routeId:this.routeId,keySource:this.keySource,path,method,requestBody,status:null,responseHeaders:{},responseBody:null,startedAt,observedAt,error:`Timeout after ${this.timeoutMs}ms`});
