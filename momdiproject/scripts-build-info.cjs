...
function sourceHash(root=process.cwd()){
 ...
 for(const name of ['package.json','package-lock.json','config.json','scripts-copy-src.cjs','scripts-build-info.cjs','scripts-start.cjs','scripts-clean.cjs'])if(fs.existsSync(path.join(root,name)))files.push(path.join(root,name));
