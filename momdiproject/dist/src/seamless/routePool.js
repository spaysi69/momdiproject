...
function proxyEndpoint(value){if(!value)return null;try{const u=new URL(value);return `${u.protocol}//${u.hostname}:${u.port|| (u.protocol==='https:'?'443':'80')}`}catch{return 'configured'}}
class SeamlessRoutePool{
  ...
  async probeRoute(route,{egress=true}={}){
    ...
    return{id:route.id,keySource:route.source,priority:route.priority,proxyConfigured:Boolean(route.proxyUrl),proxyEndpoint:proxyEndpoint(route.proxyUrl),creditsRemaining:Number.isFinite(credits?.creditsRemaining)?credits.creditsRemaining:null,lastKnownCreditsRemaining:route.lastCredits,rateLimitRemaining:credits?.rateLimitRemaining??null,rateLimitReset:credits?.rateLimitReset??null,observedAt:Number.isFinite(credits?.creditsRemaining)?credits.observedAt:null,egressIp,egressObservedAt,status:error?'ERROR':'READY',error};
  }
  ...
  async status(){
    ...
    const ipCounts=new Map();for(const item of items){if(item.egressIp)ipCounts.set(item.egressIp,(ipCounts.get(item.egressIp)||0)+1)}
    if(this.requireUniqueEgress&&items.length>1){items=items.map(item=>{if(!item.proxyConfigured)return{...item,status:'ERROR',error:'Dedicated egress proxy/NAT is required for this Seamless key.'};if(!item.egressIp)return{...item,status:'ERROR',error:item.error||'Could not verify this route public egress IP.'};if((ipCounts.get(item.egressIp)||0)>1)return{...item,status:'ERROR',error:`Duplicate egress IP ${item.egressIp}. This route is not isolated from the other Seamless organization.`};return item})}
    ...
    return{status:valid.length===items.length&&uniqueEgressOk?'READY':'ERROR',routes:items,totalCreditsRemaining:known.length?total:null,organizationCount:items.length,activeRouteId:items.find(x=>x.usageState==='ACTIVE')?.id||null,uniqueEgressRequired:this.requireUniqueEgress,uniqueEgressVerified:items.length<=1?true:(ips.length===items.length&&uniqueIps.size===items.length),error:!uniqueEgressOk?'Two Seamless organizations must use different verified public egress IPs. Check the PRIMARY and SECONDARY proxy/NAT configuration.':undefined};
  }
