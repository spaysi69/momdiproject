...
function seamlessCredentials(env=process.env){
  ...
  // Multi-organization mode is intentionally strict: every key must have its own egress route.
  // A single Render service cannot manufacture distinct public IPs, so each key must be bound
  // to a separately configured proxy/NAT endpoint and those routes are verified at runtime.
  const requireUniqueEgress=routes.length>1||/^(1|true|yes|on)$/i.test(clean(env.SEAMLESS_REQUIRE_UNIQUE_EGRESS));
  if(requireUniqueEgress&&routes.length>1){
    const missing=routes.filter(r=>!r.proxyUrl).map(r=>r.proxySource||r.source);
    if(missing.length)throw new Error(`Multi-organization mode requires distinct egress. Configure ${missing.join(', ')} with separate proxy/NAT endpoints.`);
    const proxies=routes.map(r=>r.proxyUrl);if(new Set(proxies).size!==proxies.length)throw new Error('Multi-organization mode requires distinct egress, but two Seamless keys are configured with the same proxy URL.');
  }
