...
const ResearchAcceptedSchema = zod_1.z.object({
    ...
    requestIds: zod_1.z.array(zod_1.z.union([zod_1.z.string(), zod_1.z.number()])).min(1).optional(),
    data: zod_1.z.any().optional(),
}).passthrough();
const PollEnvelopeSchema = zod_1.z.any();
...
function mapContact(contact, fallbackLinkedInUrl, extra) {
...
function readPollResult(payload) {
    const root = payload;
    const candidates = [];
    ...
    let contact = result.contact ?? result.contactData ?? result.contact_data;
    ...
    if (Array.isArray(contact))
        contact = contact[0];
    const mappedContact = contact && typeof contact === 'object' ? contact : undefined;
    return {
        status,
        message,
...
function extractRequestId(payload) {
    ...
    if ((typeof directId === 'string' && directId.trim()) || (typeof directId === 'number' && Number.isFinite(directId)))
        return String(directId);
    const first = ids.find((v) => (typeof v === 'string' && v.trim()) || (typeof v === 'number' && Number.isFinite(v)));
    if (first !== undefined)
        return String(first);
...
class SeamlessClient {
    ...
    async resolveCompanyByLinkedInUrl(companyLinkedInUrl, limit = 20) {
        ...
        if (!slug)
            throw new Error('Enter a valid LinkedIn company URL.');
        ...
        const matches = parsed.data.map(item => item).filter(company => {
        ...
        if (!company)
            throw new Error('That LinkedIn company URL could not be matched to a Seamless company record.');
        return { company };
    }
    async searchPersonByLinkedInUrl(linkedinUrl, limit = 50) {
        ...
        const queries = [
            { nextToken: null, limit: Math.min(Math.max(10, limit), 50), fullName: [humanName] },
            { nextToken: null, limit: Math.min(Math.max(10, limit), 50), contactKeyword: [slug] },
        ];
        let lastError;
        for (const body of queries) {
            try {
                ...
                const parsed = SearchEnvelopeSchema.parse(response.data);
                const matches = parsed.data.map((item) => item).filter((item) => {
                    const candidate = firstString(item.lIProfileUrl, item.liProfileUrl, item.linkedinUrl);
        ...
        throw new Error('No matching contact was found by the free contact search API for that LinkedIn profile. No research credit was consumed.');
    }
    async researchContactByIdentity(input, onAccepted) {
        ...
        for (let poll = 1; poll <= this.options.maxPolls; poll += 1) {
            ...
            if (researchFailed(status))
                throw new Error(result.message || `Seamless contact research ended with status: ${status || 'unknown'}`);
    ...
    async submitContactResearchByLinkedInUrl(linkedinUrl, skipDeduplicationCheck = false) {
        const accepted = await this.client.post('/contacts/research', {
    ...
    async pollContactResearchRequest(requestId, fallbackLinkedInUrl) {
        const response = await this.client.get('/contacts/research/poll', {
            params: { requestIds: requestId },
        });
        this.throwIfBad(response, 200, 'contact research poll');
    ...
    async researchContactByLinkedInUrl(linkedinUrl, skipDeduplicationCheck = false) {
        ...
        for (let poll = 1; poll <= this.options.maxPolls; poll += 1) {
            ...
            if (researchFailed(status))
                throw new Error(result.message || `Seamless research ended with status: ${status || 'unknown'}`);
            await new Promise(resolve => setTimeout(resolve, this.options.pollIntervalMs));
        }
        const error = new Error('Seamless contact research is still processing. Please retry rather than waiting for a long-running request.');
        error.code = 'ETIMEDOUT';
