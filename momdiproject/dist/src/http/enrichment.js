...
function selectContactPhones(contact,countryAbbr){
  const rows=[1,2].map(i=>({
    value:normalizePhone(contact?.[`contactPhone${i}`],countryAbbr),
    type:(str(contact?.[`contactPhone${i}DataType`])||'').toLowerCase(),
    score:aiScore(contact?.[`contactPhone${i}TotalAI`]),
    order:i,
    sourceField:`contactPhone${i}`
  })).filter(x=>x.value);
  const dedup=[];const seen=new Set();for(const row of rows){if(seen.has(row.value))continue;seen.add(row.value);dedup.push(row)}
  const preferred=dedup.filter(x=>!['main','company','office','switchboard'].includes(x.type));
  return (preferred.length?preferred:dedup).sort((a,b)=>b.score-a.score||a.order-b.order).slice(0,2).map(x=>x.value);
}
function emailDomain(value){const email=str(value);if(!email||!email.includes('@'))return'';return email.split('@').pop().toLowerCase().replace(/^www\./,'')}
function normalizedDomain(value){let d=str(value);if(!d)return'';d=d.toLowerCase().trim();try{if(/^https?:\/\//.test(d))d=new URL(d).hostname}catch{}return d.replace(/^www\./,'').replace(/\/.*$/,'')}
function buildContactMethods(contact,currentCompany){
  ...
  const companyDomain=normalizedDomain(str(contact?.companyDomain,contact?.emailDomain,contact?.website));
  const methods=[];const seen=new Set();
  const add=(kind,value,scope,sourceField,{companyName=null,label=null,score=0}={})=>{if(!value)return;const key=`${kind}:${String(value).toLowerCase()}`;if(seen.has(key))return;seen.add(key);methods.push({kind,value,scope,sourceField,companyName:companyName||null,label:label||null,score:Number(score)||0})};
  for(let i=1;i<=2;i++){
    const value=normalizePhone(contact?.[`contactPhone${i}`],personCountry);if(!value)continue;
    const type=(str(contact?.[`contactPhone${i}DataType`])||'contact').toLowerCase();
    add('phone',value,'person',`contactPhone${i}`,{label:type,score:aiScore(contact?.[`contactPhone${i}TotalAI`])});
  }
  for(let i=1;i<=3;i++){
    const value=normalizePhone(contact?.[`companyPhone${i}`],companyCountry);if(!value)continue;
    add('phone',value,'current_company',`companyPhone${i}`,{companyName:currentCompany,label:'company line',score:aiScore(contact?.[`companyPhone${i}TotalAI`])});
  }
  const personal=(str(contact?.personalEmail)||'').toLowerCase();if(personal)add('email',personal,'person','personalEmail',{label:'personal email'});
  const emailRows=[['email',contact?.email],['email1',contact?.email1],['email2',contact?.email2],['email3',contact?.email3]];
  for(const [field,raw] of emailRows){const value=(str(raw)||'').toLowerCase();if(!value)continue;if(personal&&value===personal){add('email',value,'person',field,{label:'personal email'});continue}const domain=emailDomain(value);if(companyDomain&&domain===companyDomain)add('email',value,'current_company',field,{companyName:currentCompany,label:'work email',score:aiScore(contact?.[`${field}TotalAI`])});else add('email',value,'unassigned',field,{label:'company not confirmed by Seamless',score:aiScore(contact?.[`${field}TotalAI`])})}
  return methods;
}
...
function parseContact(contact,fallbackUrl){
  ...
  return{
    ...
    contactMethods:methods,personMethods,currentCompanyMethods:companyMethods,unassignedMethods,
...
class EnrichmentService{
  ...
  async routePreflight(requestedRouteId){
    const route=await this.directResearchRoute(str(requestedRouteId));let beforeCredits=route.lastCredits??null,beforeObservedAt=route.lastCreditsAt??null,beforeDiagnostic=route.lastCreditDiagnostic||null;
    if(!this.pool||requestedRouteId){try{const before=await (route.rest||route.client).getCredits();beforeCredits=before.creditsRemaining;beforeObservedAt=before.observedAt;beforeDiagnostic=before.diagnostic||null}catch(error){beforeDiagnostic=error?.diagnostic||beforeDiagnostic}}
    return{route,beforeCredits,beforeObservedAt,beforeDiagnostic};
  }
  async submitOnRoute(key,baseClaim,route,beforeCredits,beforeObservedAt,{failoverFrom=null}={}){
    ...
    let response;try{response=await rest.researchLinkedIn(claim.normalizedUrl)}catch(error){if(error?.diagnostic){claim=appendTrace(claim,error.diagnostic);await this.store.saveResearch(key,claim).catch(()=>{})}throw error}
  ...
  async handleSubmissionError(key,claim,error){
    ...
    if(error instanceof SeamlessRestError&&[400,401,403,404,422,429].includes(error.status)){await this.store.deleteResearch(key).catch(()=>{});error.diagnosticTrace=traceList(traced.diagnosticTrace);throw error}
    const safe={...traced,status:'needs_review',updatedAt:Date.now(),afterCredits:error instanceof SeamlessRestError?error.creditsRemaining??null:null,message:`${error?.message||'The Seamless submission outcome is uncertain.'} No automatic retry was made.`};await this.store.saveResearch(key,safe).catch(()=>{});throw error
  }
  async pollResearch(input){
    const url=normalizeLinkedInUrl(typeof input==='string'?input:input?.linkedinUrl);const key=researchKey(url);let job=await this.store.getResearch(key);if(!job)return{status:'not_found'};if(job.status!=='processing'||job.requestIds?.length!==1)return this.publicJob(job);
    const route=this.routeForId(job.routeId);const rest=route.rest||route.client;let response;try{response=await rest.pollContactResearch(job.requestIds)}catch(error){if(error?.diagnostic){job=appendTrace(job,error.diagnostic);await this.store.saveResearch(key,job).catch(()=>{})}throw error}
    job=appendTrace(job,response.diagnostic);
    const rows=Array.isArray(response.data?.data)?response.data.data:[];const row=rows.find(item=>item?.requestId===job.requestIds[0])||rows[0];
    ...
    if(providerStatus==='done'||providerStatus==='duplicate'){
      ...
      let freshCredits=response.creditsRemaining??null,freshObservedAt=response.observedAt;try{const fresh=await rest.getCredits();if(fresh.diagnostic)job=appendTrace(job,fresh.diagnostic);if(Number.isFinite(fresh.creditsRemaining)){freshCredits=fresh.creditsRemaining;freshObservedAt=fresh.observedAt;if(this.pool)this.pool.noteCredits(route,fresh)}}catch(error){if(error?.diagnostic)job=appendTrace(job,error.diagnostic)}
  ...
  async status(){
    const result={architecture:'exact-linkedin-one-page-priority-failover',keySource:this.keySource,database:{status:'ERROR'},seamless:{status:'CONFIGURED',transport:'REST direct LinkedIn research',mcpRequired:false,contactSearchDisabled:true,maxChargeableSubmissions:1},credits:null};
