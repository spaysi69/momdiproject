...
function resetCompanySelection(clearCompanies=true){
 ...
 if(clearCompanies)$('companyResults').innerHTML='<div class="empty" style="padding:18px 8px">Find a person — Seamless contact search is read-only and does not consume research credits.</div>';
}
async function searchCompanies(){
 ...
 try{
   const data=await api('/v1/person/search',{method:'POST',body:JSON.stringify({linkedinUrl:url})});
   renderPerson(data.person||{linkedinUrl:url},data.companies||[],Boolean(data.cached));
   $('contactStepNote').textContent=data.cached?'loaded from saved free search':'person found';
   showSuccess(data.cached?'Loaded saved free-search data.':'Person found — no research credit was consumed. Choose the returned record.');
 }catch(e){
...
async function selectCompany(company){
 selectedCompany=company; selectedPersonTitle=company.title||selectedPerson?.title||''; selectedCompany.searchResultId=company.searchResultId;
 ...
 $('selectionNote').textContent='This is the Seamless search record returned for this LinkedIn profile. Researching it uses 1 Universal Credit.';
...
async function enrichSelected(){
 ...
 try{
   const data=await api('/v1/person/research',{
     method:'POST',
     body:JSON.stringify({
       linkedinUrl:selectedPerson.linkedinUrl||$('companyQuery').value.trim(),
       searchResultId:selectedCompany.searchResultId,
   ...
   $('lastUpdated').textContent=data.cached?'Loaded saved enrichment':'Enriched successfully — 1 Universal Credit used';
   refreshStatus();
 }catch(e){
...
function addResult(data,cached=false,source='provider'){
 ...
 const fields=[['Email',data.email],['Phone',data.phone],['Company',company],['LinkedIn',data.linkedinUrl||selectedPerson?.linkedinUrl]].filter(([,v])=>v);
