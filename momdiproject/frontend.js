...
async function searchCompanies(){
 ...
 if(!valid){showError('Enter a valid LinkedIn person URL, for example https://www.linkedin.com/in/example/');return}
 selectedPerson=null;selectedCompany=null;selectedPersonTitle='';
 $('companyBox').innerHTML=''; $('contactsList').innerHTML=''; $('contactsStep').classList.remove('active'); $('contactsStepNote').textContent='select a company record first'; $('enrichSelectedBtn').disabled=true;
 ...
 try{
   ...
   if(data?.status==='processing'){
     $('companyBox').innerHTML='<div class="career-intro">Seamless is researching this person…</div><div class="career-list"><div class="career-card"><strong>Research running</strong><span>This is tracked server-side, so you can refresh or search another person without starting the job again.</span></div></div>';
     let tries=0;
     while(tries<18){
   ...
   $('companyStepNote').textContent=data.cached?'loaded from saved data':'person found';
