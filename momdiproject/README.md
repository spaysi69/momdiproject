...
## Supabase company-specific cache

The app checks the persistent Supabase cache before calling Seamless. For exact person+company persistence across multiple company records, run the `supabase/schema.sql` migration to create `enrichment_person_companies`. The app remains backward-compatible with the original `enrichment_profiles` table if that migration has not yet been applied.
