# v30 release instructions

This package includes matching source and compiled dist output. Install dependencies with npm ci; Render build command remains npm ci --no-audit --no-fund && npm run build; start command is npm start. Builds clean dist first. npm start verifies compiled metadata against current source and stops if they differ. Docker compiles a fresh build within the image.

The sidebar and /health show the compiled version and build fingerprint. A server running an older source checkout cannot update itself from this download. This package has not been deployed to your Render service from this environment.

View credits now shows labeled provider balances separately and offers the original response for inspection. It never sums overlapping allocations or picks the first bucket as a total. Your true balance still depends on the response from your selected Seamless key.

LinkedIn lookup retains the v29.1 schema recognition and Copy search diagnostic action. Actual URL support remains unverified until that live diagnostic is inspected; no paid fallback is used. No new Supabase migration is required.

Verification: production build and 35 tests passed, including stale-build rejection and credit-bucket handling. Live provider behavior is unverified.

---

# Prospecting Beast 30
