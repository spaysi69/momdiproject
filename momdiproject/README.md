...
## Render Redis connection (important)

The web service must receive `REDIS_URL` from the Render Key Value service. When using the included `render.yaml` as a Blueprint, Render wires this automatically through `fromService`. If you created the web service manually, open the Key Value service → **Connect** → copy its **Internal URL**, then add it to the web service as `REDIS_URL`.

Do not set `REDIS_URL` to `localhost` on Render. The application now fails fast with a clear message instead of repeatedly trying `localhost:6379`. Keep the web service and Key Value service in the same Render region so the internal connection works.
