...
## Provider credit source
The app records the provider-reported `X-PublicAPI-Credits` response header per Seamless route. The dashboard labels a balance as real only after a provider response has supplied that header. A manual “Refresh real balances” button performs a non-research Contact Search request per configured route to refresh that provider-reported balance.

## Seamless MCP search
The Find person step uses Seamless MCP `search_contacts`, a read-only search. Enable MCP scope on the API key and set `SEAMLESS_MCP_API_KEY_PRIMARY` / `SEAMLESS_MCP_API_KEY_SECONDARY` if you use separate keys.
