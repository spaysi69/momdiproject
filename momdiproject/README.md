...
## Diagnostic test build (40.1.0-test)

This package is intended for short-lived provider debugging.

After enriching a NEW LinkedIn profile, open **Seamless diagnostic trace** on the result card and click **Copy diagnostic report**. Paste that JSON into the debugging conversation.

The report includes:
- selected Seamless route/key variable name (never the key value),
- preflight credit probe request/response,
- exact `/contacts/research` request body,
- research HTTP status and response headers/body,
- every `/contacts/research/poll` response,
- final credit probe,
- before/after `X-PublicAPI-Credits`,
- returned `requestIds`, provider status, raw returned contact object, and Beast's parsed contact.

Secrets (`Token`, authorization, API keys, passwords, secrets) are redacted automatically. Contact data is intentionally NOT redacted because the purpose of this test is to inspect exactly what Seamless returned. Treat copied reports as personal data.

Diagnostic capture makes no additional Seamless calls. The normal 10-second background credit refresh is disabled in this test build to keep an enrichment trace easy to interpret.
