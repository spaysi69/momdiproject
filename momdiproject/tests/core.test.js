...
test('status declares exact single-research architecture with contact search disabled',async()=>{const store=new MemoryStore();const svc=new EnrichmentService(cfg,store,{getCredits:async()=>({creditsRemaining:44,observedAt:'now'})});const r=await svc.status();assert.equal(r.architecture,'exact-linkedin-deep-diagnostic-test');assert.equal(r.seamless.contactSearchDisabled,true);assert.equal(r.seamless.mcpRequired,false);assert.equal(r.seamless.maxChargeableSubmissions,1)});
...
test('verified build fingerprint matches diagnostic source',()=>{const {sourceHash}=require('../scripts-build-info.cjs');const info=JSON.parse(fs.readFileSync(path.join(__dirname,'../dist/BUILD_INFO.json'),'utf8'));assert.equal(info.version,'41.0.0-deep-test');assert.equal(info.sourceHash,sourceHash())});
...
test('diagnostic UI exposes a copyable provider report and warns that contact data is preserved',()=>{
  ...
  assert.match(html,/DEEP TEST TRACE/);
