...
test('status declares exact single-research architecture with contact search disabled',async()=>{const store=new MemoryStore();const svc=new EnrichmentService(cfg,store,{getCredits:async()=>({creditsRemaining:44,observedAt:'now'})});const r=await svc.status();assert.equal(r.architecture,'exact-linkedin-production');assert.equal(r.seamless.contactSearchDisabled,true);assert.equal(r.seamless.mcpRequired,false);assert.equal(r.seamless.maxChargeableSubmissions,1)});
...
test('contact method classification never invents historical-company associations',()=>{const built=buildContactMethods({companyDomain:'newco.com',email:'jane@newco.com',email1:'jane@newco.com',email1Selected:true,email1EmailAI:'valid',email1TotalAI:'98%',email2:'jane@oldco.com',email2EmailAI:'invalid',personalEmail:'jane@gmail.com',contactPhone1:'4155550101',contactPhone1DataType:'mobile',companyPhone1:'6505550200',contactLocation:{countryAbbr:'US'},companyLocation:{countryAbbr:'US'}},'NewCo');const work=built.methods.find(x=>x.value==='jane@newco.com');const rejected=built.other.find(x=>x.value==='jane@oldco.com');const personal=built.methods.find(x=>x.value==='jane@gmail.com');const companyPhone=built.methods.find(x=>x.value==='+16505550200');assert.equal(work.scope,'current_company');assert.equal(work.companyName,'NewCo');assert.equal(work.selected,true);assert.equal(rejected.scope,'provider_candidate');assert.equal(rejected.status,'invalid');assert.equal(personal.scope,'person');assert.equal(companyPhone.scope,'current_company');assert.equal(companyPhone.companyName,'NewCo')});

test('UI preserves current-company, personal and provider-candidate sections',()=>{const js=fs.readFileSync(path.join(__dirname,'../src/server/public/app.js'),'utf8');assert.match(js,/Current company/);assert.match(js,/Personal \/ direct/);assert.match(js,/Other provider candidates/);assert.match(js,/No company-specific phone\/email mapping was supplied/)});
...
test('verified build fingerprint matches diagnostic source',()=>{const {sourceHash}=require('../scripts-build-info.cjs');const info=JSON.parse(fs.readFileSync(path.join(__dirname,'../dist/BUILD_INFO.json'),'utf8'));assert.equal(info.version,require('../package.json').version);assert.equal(info.sourceHash,sourceHash())});
...
test('v42 runtime has no egress or proxy requirement',()=>{const files=['../src/config/credentials.js','../src/seamless/routePool.js','../src/seamless/restClient.js','../src/server/public/app.js','../src/server/public/chatbox.html'];const joined=files.map(f=>fs.readFileSync(path.join(__dirname,f),'utf8')).join('\n');assert.equal(joined.includes('SEAMLESS_EGRESS'),false);assert.equal(joined.toLowerCase().includes('egress ip'),false);assert.equal(fs.existsSync(path.join(__dirname,'../src/seamless/proxyTransport.js')),false)});

test('v42 UI keeps results in a scroll-snap feed and appends new slides',()=>{const html=fs.readFileSync(path.join(__dirname,'../src/server/public/chatbox.html'),'utf8');const js=fs.readFileSync(path.join(__dirname,'../src/server/public/app.js'),'utf8');const css=fs.readFileSync(path.join(__dirname,'../src/server/public/app.css'),'utf8');assert.match(html,/id="result-feed"/);assert.match(js,/feed\.append\(slide\)/);assert.match(js,/scrollIntoView/);assert.match(css,/scroll-snap-type:y mandatory/);assert.match(css,/scroll-snap-align:start/)});

test('production credits use fresh Seamless org headers and refresh every 15 seconds',()=>{const js=fs.readFileSync(path.join(__dirname,'../src/server/public/app.js'),'utf8');assert.match(js,/X-PublicAPI-Credits/);assert.match(js,/total withheld because every configured organization did not return a fresh credit header/);assert.match(js,/Local fallback<\/span><strong>Never used as fresh balance/);assert.match(js,/auto_15s/);assert.match(js,/15000/);assert.equal(/10000/.test(js),false)});


test('diagnostic-derived contact quality keeps only valid emails and plausible person phones in main card',()=>{
  const d=parseContact({fullName:'Mikel Smith',company:'Anatomy IT',email:'mikel.smith@anatomyit.com',email1:'mikel.smith@anatomyit.com',email1Selected:true,email1TotalAI:'98%',email1EmailAI:'valid',email2:'msmith@anatomyit.com',email2TotalAI:'97%',email2EmailAI:'valid',email3:'mikel@anatomyit.com',email3TotalAI:'0%',email3EmailAI:'invalid',contactPhone1:'718.289.4974',contactPhone1TotalAI:'80%',contactPhone1DataType:'mobile',contactPhone2:'91468300052114',contactPhone2DataType:'main',contactPhone3:'845.337.2057',contactPhone3DataType:'mobile',companyPhone1:'704.523.3877',companyPhone1TotalAI:'99%',companyPhone2:'914.683.0005',companyPhone2TotalAI:'87%',companyPhone3:'914.741.6500',companyPhone3TotalAI:'37%',contactLocation:{countryAbbr:'US'},companyLocation:{countryAbbr:'US'}},'https://www.linkedin.com/in/mikel-smith/');
  assert.deepEqual(d.phones,['+17182894974','+18453372057']);
  assert.equal(d.currentCompanyMethods.some(x=>x.value==='mikel.smith@anatomyit.com'&&x.selected&&x.score===98),true);
  assert.equal(d.currentCompanyMethods.some(x=>x.value==='msmith@anatomyit.com'&&x.status==='valid'&&x.score===97),true);
  assert.equal(d.currentCompanyMethods.some(x=>x.value==='mikel@anatomyit.com'),false);
  assert.equal(d.otherCandidates.some(x=>x.value==='mikel@anatomyit.com'&&x.status==='invalid'),true);
  assert.equal(d.otherCandidates.some(x=>x.rawValue==='91468300052114'),true);
  assert.equal(d.currentCompanyMethods.some(x=>x.value==='+17045233877'&&x.score===99),true);
  assert.equal(d.currentCompanyMethods.some(x=>x.value==='+19146830005'&&x.score===87),true);
  assert.equal(d.otherCandidates.some(x=>x.value==='+19147416500'&&x.score===37),true);
});

test('US phone plausibility rejects oversized numbers and accepts contactPhone3',()=>{assert.equal(normalizePhone('91468300052114','US'),null);assert.equal(normalizePhone('845.337.2057','US'),'+18453372057')});

test('UI displays requested 0+ international phone format without corrupting tel value',()=>{const js=fs.readFileSync(path.join(__dirname,'../src/server/public/app.js'),'utf8');assert.match(js,/startsWith\('\+'\)\?`0\$\{value\}`/);assert.match(js,/href=\"tel:\$\{esc\(m\.value\)\}\"/) });
...
test('diagnostic UI exposes a copyable provider report and warns that contact data is preserved',()=>{
  ...
  assert.match(js,/Copy FULL diagnostic report/);
  assert.match(js,/\/v1\/diagnostics\/report/);
  assert.match(js,/API\/auth secrets are redacted/);
  assert.match(html,/PROVIDER AUDIT/);
  assert.match(html,/Download JSON/);
});


test('deep diagnostic journal captures credit-indicator probes for both organizations without research',async()=>{
  const {DiagnosticJournal}=require('../src/diagnostics/journal');
  const journal=new DiagnosticJournal();
  const env={SEAMLESS_API_KEY_PRIMARY:'p',SEAMLESS_API_KEY_SECONDARY:'s'};
  const balances={primary:78,secondary:75};
  const pool=new SeamlessRoutePool(cfg,env,route=>new SeamlessRestClient(route.key,cfg.apiBaseUrl,30000,async()=>response({success:true,data:[]},200,{'X-PublicAPI-Credits':String(balances[route.id]),'X-RateLimit-Remaining':'50'}),{routeId:route.id,keySource:route.source,onDiagnostic:r=>journal.add('provider_http','request_complete',r)}),journal);
  const svc=new EnrichmentService(cfg,new MemoryStore(),null,pool,journal);
  const st=await svc.status({reason:'credits_click'});
  assert.equal(st.credits.totalCreditsRemaining,153);
  const report=svc.diagnosticReport();
  assert.equal(report.derived.researchPostCount,0);
  assert.equal(report.derived.balanceReadCount,2);
  assert.equal(report.derived.providerCreditSeries.primary.at(-1).credits,78);
  assert.equal(report.derived.providerCreditSeries.secondary.at(-1).credits,75);
  assert.equal(report.events.some(e=>e.category==='status'&&e.event==='application_status_requested'&&e.details.reason==='credits_click'),true);
});

test('deep diagnostic report derives one research POST, one submitted contact and credit jump',()=>{
  const {DiagnosticJournal}=require('../src/diagnostics/journal');
  const journal=new DiagnosticJournal();
  const svc=new EnrichmentService(cfg,new MemoryStore(),{getCredits:async()=>({creditsRemaining:10})},null,journal);
  journal.add('provider_http','request_complete',{routeId:'primary',keySource:'SEAMLESS_API_KEY_PRIMARY',request:{method:'GET',path:'/contacts?page=1'},response:{headers:{'x-publicapi-credits':'78'},status:200}});
  journal.add('provider_http','request_complete',{routeId:'primary',keySource:'SEAMLESS_API_KEY_PRIMARY',request:{method:'POST',path:'/contacts/research',body:{contacts:[{liProfileUrl:'https://www.linkedin.com/in/jane/'}],skipDeduplicationCheck:false}},response:{headers:{'x-publicapi-credits':'77'},status:202,body:{requestIds:['req1']}}});
  journal.add('provider_http','request_complete',{routeId:'primary',keySource:'SEAMLESS_API_KEY_PRIMARY',request:{method:'GET',path:'/contacts/research/poll?requestIds=req1'},response:{headers:{'x-publicapi-credits':'77'},status:200}});
  const r=svc.diagnosticReport();
  assert.equal(r.derived.researchPostCount,1);
  assert.equal(r.derived.submittedContacts,1);
  assert.equal(r.derived.pollCount,1);
  assert.deepEqual(r.derived.requestIds,['req1']);
  assert.equal(r.derived.creditJumps.some(x=>x.from===78&&x.to===77&&x.delta===1),true);
  assert.equal(r.derived.invariants.oneContactPerResearchPost,true);
  assert.equal(r.derived.invariants.deduplicationCheckNeverSkipped,true);
});

test('diagnostic reset and report endpoints do not themselves call Seamless research',async()=>{
  let providerCalls=0;const journal=require('../src/diagnostics/journal').DiagnosticJournal;const j=new journal();
  const store=new MemoryStore();const svc=new EnrichmentService(cfg,store,{getCredits:async()=>{providerCalls++;return{creditsRemaining:10}}},null,j);
  const old=process.env.APP_AUTH_TOKEN;process.env.APP_AUTH_TOKEN='pw';const server=http.createServer(createHandler(svc));await new Promise(r=>server.listen(0,'127.0.0.1',r));const base=`http://127.0.0.1:${server.address().port}`;
  try{let res=await fetch(base+'/v1/diagnostics/reset',{method:'POST',headers:{Authorization:'Bearer pw','Content-Type':'application/json'},body:JSON.stringify({reason:'test'})});assert.equal(res.status,200);res=await fetch(base+'/v1/diagnostics/report',{headers:{Authorization:'Bearer pw'}});assert.equal(res.status,200);const body=await res.json();assert.equal(body.derived.researchPostCount,0);assert.equal(providerCalls,0)}finally{await new Promise(r=>server.close(r));if(old===undefined)delete process.env.APP_AUTH_TOKEN;else process.env.APP_AUTH_TOKEN=old}
});


test('v48 token drawer hides provider API branding and exposes token terminology',()=>{
  const html=fs.readFileSync(path.join(__dirname,'../src/server/public/chatbox.html'),'utf8');
  const js=fs.readFileSync(path.join(__dirname,'../src/server/public/app.js'),'utf8');
  assert.match(html,/TOKEN BALANCE/);
  assert.match(js,/Primary token/);
  assert.match(js,/Secondary token/);
  assert.doesNotMatch(html+js,/SEAMLESS BALANCE|SEAMLESS_API_KEY|Seamless API key|configured Seamless/i);
});

test('v48 premium UI keeps motion accessible and adds both enrichment and prospecting transitions',()=>{
  const css=fs.readFileSync(path.join(__dirname,'../src/server/public/app.css'),'utf8');
  assert.match(css,/slideInPremium/);
  assert.match(css,/prospectRowIn/);
  assert.match(css,/ambient-layer/);
