...
test('verified build fingerprint matches diagnostic source',()=>{const {sourceHash}=require('../scripts-build-info.cjs');const info=JSON.parse(fs.readFileSync(path.join(__dirname,'../dist/BUILD_INFO.json'),'utf8'));assert.equal(info.version,'42.0.0');assert.equal(info.sourceHash,sourceHash())});
